#!/bin/bash
docker exec -it replication-master mysql -u root -p@H9p3v6c -e CREATE USER 'ssl_test'@'%' IDENTIFIED BY '123' REQUIRE SSL;
docker exec -it replication-master mysql -u root -p@H9p3v6c -e GRANT REPLICATION SLAVE ON *.* TO 'ssl_test'@'%';
